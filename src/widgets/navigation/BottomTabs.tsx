import { BookOpen, CalendarRange, CircleUserRound, Heart, House, MessageCircle } from 'lucide-react';
...
const tabs = [
  ...
  { to: '/community', label: '커뮤니티', icon: BookOpen, match: ['/community'] },
    ...
    <nav className="safe-bottom fixed bottom-0 left-0 right-0 z-40 border-t border-black/5 bg-white/94 backdrop-blur-xl md:hidden">
      <div className="grid grid-cols-6 px-2 pt-2">
        {tabs.map((tab) => {
          ...
          return (
            <Link key={tab.to} to={tab.to} aria-current={active ? 'page' : undefined} style={active ? { color: 'var(--orange)' } : undefined} className={cn('flex min-h-14 flex-col items-center justify-center gap-1 px-1 text-[10px] font-black', !active && 'text-slate-400')}>
