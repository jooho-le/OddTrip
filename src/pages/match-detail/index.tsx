...
export function MatchDetailPage() {
  ...
  return (
    <div className="page-canvas space-y-5">
      ...
      <Link to="/decision" onClick={() => void ensureTrip()}><Button icon={<Heart className="h-4 w-4" />} className="w-full md:w-auto" disabled={status.trip === 'loading'}>{status.trip === 'loading' ? '공동 여행 생성 중' : '공동 일정 만들기'}</Button></Link>
    </div>
  );
}
