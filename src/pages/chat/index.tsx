...
export function ChatPage() {
  ...
  const loadRooms = useCallback(async () => {
    try { setRooms((await chatService.listRooms()).data.items); } catch (reason) { setError(messageOf(reason)); }
  }, []);
  ...
  useEffect(() => {
    ...
    socket.onmessage = (message) => {
      try {
        ...
        if (event.event === 'message.created' || event.event === 'message.deleted' || ((event.event === 'match.ended' || event.event === 'user.blocked') && affectsMatch)) {
  ...
  return <div className="page-canvas space-y-5">
    <div><h1 className="text-4xl font-black">채팅</h1><p className="mt-2 text-sm font-bold text-slate-500">수락된 동행과 여행을 조율합니다.</p></div>
