...
import { LoadState, PageHeading, Panel, StatusPill, TableShell, formatDate, formatDateTime, tdClass, thClass } from '../ui';
    ...
    <>
      ...
      <Panel>
        <form
          ...
          onSubmit={(event: FormEvent) => { event.preventDefault(); setApplied(query); }}
        >
...
function SanctionForm({ userId, onDone }: { userId: string; onDone: () => void }) {
  ...
  // 되돌릴 수 없는 조치는 한 번 더 묻는다.
  const irreversible = type === 'withdrawal';

  const submit = async (event: FormEvent) => {
    event.preventDefault();
    if (irreversible && !confirming) { setConfirming(true); return; }
    setBusy(true);
    setError('');
    try {
  ...
  return (
    <form onSubmit={submit} className="mt-6 border border-slate-200 p-4">
      ...
      >
        {busy ? '처리 중…' : confirming ? '강제 탈퇴 실행' : '제재 부과'}
      </button>
    </form>
  );
}
