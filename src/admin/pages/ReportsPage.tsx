...
import { LoadState, PageHeading, Pagination, Panel, StatusPill, TableShell, formatDateTime, tdClass, thClass } from '../ui';
    ...
    <>
      ...
      <Panel>
        <div className="mb-5 flex flex-col gap-3 sm:flex-row">
          ...
          <select
            value={reason}
            onChange={(event) => { setReason(event.target.value); setOffset(0); }}
            className="h-12 rounded-2xl border border-slate-200 bg-white px-4 text-sm font-bold"
          >
        ...
        {reports.data?.items.length ? (
          <>
            ...
            <Pagination offset={offset} limit={pageSize} total={reports.data.total} onChange={setOffset} />
          </>
        ) : null}
      </Panel>
