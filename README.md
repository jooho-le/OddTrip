...
현재 MVP 흐름은 별도 장소 투표 대신 AI가 두 사람의 공동 선호를 반영해 일정까지 생성한다. 일정 항목 개별 수정, 실제 도로 경로 계산, SMS 본인인증, D-1·모바일 Push, 관광지·TTI 편집·운영 관리자 API는 성공 상태를 만들지 않는다. 상세 후속 계약과 검증 조건은 [`docs/BACKEND_GAPS.md`](docs/BACKEND_GAPS.md)와 [`docs/FRONTEND_REDESIGN_TODO.md`](docs/FRONTEND_REDESIGN_TODO.md)에 정리한다.

## 여행 커뮤니티 — 프론트 체험

`/community`에서 카테고리·검색·정렬·내 글·저장한 글을 탐색하고, 상세·글쓰기·수정·임시저장 목록을 이용할 수 있다. `/home`에서는 같은 저장소의 글을 추천·최신·현재 여행지 기준으로 요약하고, 글 상세와 전체 목록으로 이동한다. 네이버 블로그의 탐색·작성 흐름을 참고하되 기존 OddTrip 디자인을 유지했다.

현재 게시판 백엔드는 없으며 글·사진·댓글·공감·초안은 계정별 브라우저 저장으로만 동작한다. 첫 진입과 작성 완료 팝업에서 이 범위를 안내한다. 실제 공개·공유·신고·서버 알림은 연결되지 않는다. 구현 경로와 후속 계약은 [커뮤니티 프론트 문서](docs/COMMUNITY_FRONTEND.md)를 참고한다.
